...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    ...
    blockGuessTimeLimitInSeconds: Schema.number()
      .min(0)
      .default(0)
      .description("猜测色块的时间限制（秒），值为 0 时则不限制时间。"),
    blockSize: Schema.number()
      .default(50)
      .description("每个颜色方块的大小（像素）。"),
    spacingBetweenGrids: Schema.number()
      .default(10)
      .description("色块之间的水平与垂直间距（像素）。"),
    isNumericGuessMiddlewareEnabled: Schema.boolean()
      .default(true)
      .description("是否启用数字猜测中间件。"),
    shouldInterruptMiddlewareChainAfterTriggered: Schema.boolean()
      .default(true)
      .description("是否在触发后中断中间件链。"),
  }).description("基础配置"),
  Schema.object({
    isCompressPicture: Schema.boolean()
      .default(false)
      .description("是否压缩图片（不建议）。"),
  }).description("图片配置"),
  Schema.union([
    Schema.object({
      isCompressPicture: Schema.const(true).required(),
      pictureQuality: Schema.number()
        .min(1)
        .max(100)
        .default(80)
        .description("压缩后图片的质量（1 ~ 100）。"),
    }),
    Schema.object({}),
  ]),
  Schema.object({
    enableAutoRecall: Schema.boolean()
      .default(false)
      .description("是否启用自动撤回功能。"),
    autoRecallDelay: Schema.number()
      .min(1)
      .default(60)
      .description("自动撤回的延迟时间（秒）。需要机器人拥有相应的撤回权限。"),
  }).description("自动撤回配置"),
]) as any;
...
export function apply(ctx: Context, config: Config) {
  ...
  // 自动撤回功能的辅助函数
  async function sendAndRecall(session: Session, content: h.Fragment | string) {
    if (!config.enableAutoRecall || config.autoRecallDelay <= 0) {
    ...
    setTimeout(() => {
      messageIds.forEach((id) => {
        session.bot.deleteMessage(session.channelId, id).catch((err) => {
          logger.warn(
            `Failed to recall message ${id} in channel ${session.channelId}: ${err}`
          );
        });
      });
    }, config.autoRecallDelay * 1000);
    return messageIds;
  }
  ...
  // zl*
  // bz* h*
  ctx.command("seeColor", "给我点颜色看看帮助").action(async ({ session }) => {
    await session.execute(`seecolor -h`);
  });
  // ks* s*
  ctx.command("seeColor.开始", "开始游戏").action(async ({ session }) => {
    // 获取游戏信息
    const gameInfo = await getGameInfo(session.channelId);
    ...
    // 开始游戏
    const buffer = await generatePictureBuffer(
      config.initialLevel,
      session.channelId
    );
    await sendAndRecall(
      ...
      `${h.at(session.userId)} ~\n${msg.start}\n${h.image(
        buffer,
        `image/${config.isCompressPicture ? `jpeg` : `png`}`
      )}\n${msg.guess}`
    );
    // 更新游戏状态
  ...
  // c*
  ctx
    .command("seeColor.猜 <numberString:text>", "猜色块")
    .action(async ({ session }, numberString) => {
      ...
      if (
        timeDifference > config.blockGuessTimeLimitInSeconds &&
        config.blockGuessTimeLimitInSeconds > 0
      ) {
        await sendAndRecall(
          session,
          `时间超过 ${config.blockGuessTimeLimitInSeconds} 秒！游戏结束！😢`
        );
  ...
  // tz* s* js*
  ctx.command("seeColor.结束", "强制结束游戏").action(async ({ session }) => {
    // 获取游戏信息
    const gameInfo = await getGameInfo(session.channelId);
    if (gameInfo.isStarted) {
      await ctx.database.remove("see_color_playing_records", {
  ...
  // r* phb* sb*
  ctx.command("seeColor.排行榜", "查看色榜").action(async ({}) => {
    const rankInfo: SeeColorRank[] = await ctx.database.get(
      "see_color_rank",
      {}
    );
...
${rankInfo
  .map(
    (player, index) =>
      ` ${String(index + 1).padStart(2, " ")}   ${player.userName.padEnd(
        6,
        " "
      )} ${player.score.toString().padEnd(4, " ")}`
  )
  .join("\n")}
`;
    }
  ...
  function to256(scale: number) {
    ...
    return scale > 255
      ...
      : scale < 0
      ? "00"
      : Math.floor(scale).toString(16).padStart(2, "0");
  }
  ...
  function adjustColor(color, percentage, mode, maxIterations = 100) {
    ...
    do {
      ...
      const adjusted = [rgb >> 16, (rgb >> 8) & 0xff, rgb & 0xff].map(
        (value) => {
          let newValue;
          if (mode === "随机") {
            newValue =
      ...
      const adjustedColor = adjusted.map((value) =>
  ...
  async function updatePlayingRecord(session, gameInfo) {
    ...
    if (playingRecord.length === 0) {
      ...
      await ctx.database.set(
        "see_color_playing_records",
        { channelId: session.channelId, userId: session.userId },
        {
          username: session.username,
  ...
  async function updateRank(userId: string, userName: string, score: number) {
    ...
    if (rankInfo.length === 0) {
      await ctx.database.create("see_color_rank", {
        userId: userId,
        userName: userName,
        score: score,
      });
  ...
  async function getGameInfo(channelId: string): Promise<SeeColorGame> {
    const gameInfo = await ctx.database.get("see_color_games", {
      channelId: channelId,
    });
    if (gameInfo.length === 0) {
      return await ctx.database.create("see_color_games", {
        channelId: channelId,
        isStarted: false,
      });
    } else {
  ...
  async function generatePictureBuffer(n: number, channelId: string) {
    ...
    const canvasSize =
      n * blockSize + (n - 1) * spacingBetweenGrids + offset * 2;
    await ctx.database.set(
      "see_color_games",
      { channelId: channelId },
      { block: diffIndex + 1 }
